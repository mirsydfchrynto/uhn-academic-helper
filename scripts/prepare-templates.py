...
def clean_paragraph_placeholder(p):
    ...
    # Determine if paragraph contains our target placeholders
    is_name = text == "Nama" or text == "nama"
    is_nim = "NIM." in text and ("xxxx" in text or "Xxxx" in text or "0000" in text or "2309" in text)
    is_title = "JUDUL" in text and ("MENGGUNAKAN FONT" in text or "TIMES NEW ROMAN" in text or "UKURAN 12" in text)
    
    # Also handle some variations or cases
    if not (is_name or is_nim or is_title):
        return False
        
    replacement_text = ""
    if is_name:
        replacement_text = "{NAMA}"
        print(f"      Replacing name placeholder: '{text}' -> '{replacement_text}'")
    elif is_nim:
        replacement_text = "NIM. {NIM}"
        print(f"      Replacing NIM placeholder: '{text}' -> '{replacement_text}'")
    elif is_title:
        replacement_text = "{JUDUL}"
        print(f"      Replacing title placeholder: '{text}' -> '{replacement_text}'")
    ...
    if r_pr_style is not None:
        # We make a copy of the run properties element
...
def prepare_docx_template(src_path, dest_path):
    ...
    # Register other common namespaces found in Word files
